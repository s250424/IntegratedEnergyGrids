...
CONFIG_D ["transmission_lines"] = [
    ...
    {"name": "NL-DE_LU", "bus0": "NL",    "bus1": "DE_LU", "x": 0.1, "s_nom": 5000},
]
...
CONFIG_J["no_gas_supply"] = True # removes CH4 availability; gas-based technologies cannot dispatch
...
for year in CONFIG_B["years"]:  # loop through weather years to test sensitivity
    network_b = NetworkBuilder(CONFIG_B, input_data_b, year)
    networks[year] = network_b

    # Obtain capacities of technologies
    ...
    export_results_to_json(network_b.network, "task_b", year)

visualizer_b = Visualizer(networks[CONFIG_B["years"][0]].network, scenario_name="b") 
visualizer_b.plot_sensitivity_capacity_to_weather_years(capacity_by_tech)
visualizer_b.plot_capacity_factors(input_data_b)
...
visualizer_c.plot_dispatch_diff_time_series(other=visualizer_a, start_summer=pd.Timestamp("2023-07-01"), start_winter=pd.Timestamp("2023-12-01"))
visualizer_c.plot_annual_electricity_mix()
visualizer_c.plot_storage_behavior(start_summer=pd.Timestamp("2023-07-01"),start_winter=pd.Timestamp("2023-12-01"))
...
with open("results/numerical_results/task_f.json", "w") as f:
    json.dump(results_f, f, indent=4)

visualizer_c = Visualizer(network_c.network, scenario_name="c")
visualizer_f = Visualizer(networks_f[max(networks_f.keys())].network, scenario_name="f")
visualizer_f.plot_scenario_comparison(other=visualizer_c,self_label="C (no CO₂ limit)", other_label="F (1990 CO₂ limit)",name="comparison_c_vs_f")
visualizer_f.plot_co2_sensitivity(networks_f=networks_f, ref_co2=REF_CO2, name="co2_sensitivity")
...
visualizer_g.plot_energy_transport_comparison()
